...
def get_user_pnrs(RequestBody={}):
    """
    get_user_pnrs() => Retrieves User pnrs for a specific user from database.
    ...
    try:
        ...
        try:
            ...
            result = db_read_helper.map_response(
                response=user_pnrs, target_map=target_map
            )
            ...
            raise Exception(
                get_exception_message(
                    Ex=E, file=__file__, parent=inspect.stack()[0][3], line=inspect.stack()[
                        0][2],
                    msg="Failed to SELECT User pnrs from 'pnr' Table!"
                )
            )
