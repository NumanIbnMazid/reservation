...
def get_package_information_list():
    """
    get_package_information_list() => Retrieves package information list from database.
    """
    try:
        ...
        try:
            ...
            cur.execute(
                "SELECT id, package_name, offer_rate, num_of_days, amount, cut_off_amount, image, summary, created_at, updated_at FROM setup_packageinformation"
            )

            # fetch package information
            package_information_list = cur.fetchall()
            ...
            result = db_read_helper.map_response(
                response=package_information_list, target_map=target_map
            )
        ...
        # ------- Handle Exceptions -------
            ...
            raise Exception(
                get_exception_message(
                    ...
                    msg="Failed to SELECT Package Information List from 'setup_packageinformation' Table!"
                )
            )
        ...
        raise Exception(
            get_exception_message(
                Ex=E, file=__file__, parent=inspect.stack()[0][3], line=inspect.stack()[
                    0][2],
                msg="Failed to get Package Information List!"
            )
        )
