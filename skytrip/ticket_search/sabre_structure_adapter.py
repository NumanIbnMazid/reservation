...
class SabreStructureAdapter:
    """
    StructureAdapter => Structures the Sabre BFM Module"s Response data in SkyTrip Standard format
    """
    ...
    def __init__(self, response, search_response_structure, require_utils=True):
        # set response
        self.response = response
        # set search response structure
        self.search_response_structure = search_response_structure
    ...
    def assign_nodes_to_structure(self):
        ...
        structure["statusCode"] = self.response.get("statusCode", None)
        structure["body"]["token"] = self.response["body"].get("token", None)
        # assign messages
    ...
    def get_structure(self):
        """
        get_structure() => Gets SkyTrip Standard Ticket Search Response Structure.
        return => structure (object)
        """
        # define structure
        structure = self.search_response_structure
        # return structure
        return structure
    ...
    def generate_leg_description(self, structure={}):
        ...
        for leg_description in self.__root_node["itineraryGroups"][0]["groupDescription"].get("legDescriptions", []):
            ...
            structure["body"]["responseData"]["LegDescription"].append(
                {
                    "DepartureDate": leg_description.get("departureDate", None),
                    "DepartureLocation": leg_description.get("departureLocation", None),
                    "ArrivalLocation": leg_description.get("arrivalLocation", None)
                }
            )
    ...
    def generate_itinerary(self, itineraries_list=None):
        ...
        for itinerary in itineraries_list:
            ...
            formatted_itineraries = {
                "ID": itinerary.get("id", None),
                "GDS": "Sabre",
                "ScheduleDescription": generated_schedules,
                "PassengerInfo": generated_passenger_infos,
                # "TotalFare": {
                #     "FareCurrency": totalFare.get("currency", None),
                #     "TotalFare": totalFare.get("totalPrice", None),
                #     "BaseFare": totalFare.get("baseFareAmount", None),
                #     "Tax": totalFare.get("totalTaxAmount", None),
                #     "TaxCurrency": totalFare.get("currency", None),
                # }
                "TotalFare": totalFare
            }
            ...
            if leg.get("ref", "") in self.extracted_legDescs:
                ...
                for schedule_ref in self.legDescs[self.extracted_legDescs[leg["ref"]]].get("schedules", []):
                    ...
                    if schedule_ref.get("ref", "") in self.extracted_scheduleDescs:
                        ...
                        schedules.append(
                            {
                                "NoOfStoppage": schedule.get("stopCount", None),
                                "ETicketable": schedule.get("eTicketable", None),
                                "TotalDistanceInMiles": schedule.get("totalMilesFlown", None),
                                "Departure": {
                                    "Airport": schedule["departure"].get("airport", None),
                                    "City": schedule["departure"].get("city", None),
                                    "Country": schedule["departure"].get("country", None),
                                    "Time": schedule["departure"].get("time", None),
                                    "Terminal": schedule["departure"].get("terminal", None),
                                    "DateAdjustment": schedule_ref.get("departureDateAdjustment", 0)
                                },
                                "Arrival": {
                                    "Airport": schedule["arrival"].get("airport", None),
                                    "City": schedule["arrival"].get("city", None),
                                    "Country": schedule["arrival"].get("country", None),
                                    "Time": schedule["arrival"].get("time", None),
                                    "Terminal": schedule["arrival"].get("terminal", None),
                                    "DateAdjustment": schedule["arrival"].get("dateAdjustment", 0)
                                },
                                "Carrier": {
                                    "OperatingCarrierCode": schedule["carrier"].get("operating", None),
                                    "OperatingFlightNumber": schedule["carrier"].get("operatingFlightNumber", None),
                                    "MarketingCarrierCode": schedule["carrier"].get("marketing", None),
                                    "MarketingFlightNumber": schedule["carrier"].get("marketingFlightNumber", None)
                                }
                            }
                        )
    ...
    def generate_passenger_infos(self, passenger_info_list=None):
        ...
        for passenger in passenger_info_list:
            ...
            passenger_infos.append(
                {
                    "PassengerType": passenger_info.get("passengerType", None),
                    "PassengerNumber": passenger_info.get("passengerNumber", None),
                    "NonRefundable": passenger_info.get("nonRefundable", None),
        ...
        for fare in passenger_obj["passengerInfo"].get("fareComponents", []):
            ...
            for segment in fare.get("segments", {}):
                ...
                # Add segment to segments list to set segments for this fare
                    segments.append(
                        {
                            "CabinClass": self.get_cabin_class(cabin_class=segment["segment"].get("cabinCode", None)),
                            "SeatsAvailable": segment["segment"].get("seatsAvailable", None),
                            "BookingCode": segment["segment"].get("bookingCode", None),
                            "MealCode": segment["segment"].get("mealCode", None)
                        }
                    )
                else:
            ...
            if fare.get("ref", "") in self.extracted_fareComponentDescs:
                ...
                fare_descriptions.append(
                    {
                        "NotValidAfter": component.get("notValidAfter", None),
                        "NotValidBefore": component.get("notValidBefore", None),
                        "Segment": segments
                    }
                )
        ...
        for baggage in passenger_obj["passengerInfo"].get("baggageInformation", []):
            ...
            if baggage["allowance"].get("ref", "") in self.extracted_baggageAllowanceDescs:
                ...
                baggage_info_list.append(
                    {
                        "NumberOfPieces": allowance.get("pieceCount", None),
                        "MaxWeight": allowance.get("weight", None),
                        "Unit": allowance.get("unit", None)
                    }
                )
    ...
    def generate_passenger_total_fare(self, passenger_obj=None):
        ...
        #     "TotalFare": passenger_fare.get("totalFare", None),
        #     "TotalTaxAmount": passenger_fare.get("totalTaxAmount", None),
        #     "Currency": passenger_fare.get("currency", None),
        #     "BaseFareAmount": passenger_fare.get("baseFareAmount", None),
        #     "BaseFareCurrency": passenger_fare.get("baseFareCurrency", None)
        # }
